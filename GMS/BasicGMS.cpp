...
void BasicGMS::CreateObject() {
    ...
    mt.Notification->Prognoses->Calibration.value.DateOfCalibration =
            std::chrono::system_clock::now() - std::chrono::hours(24 * 7);
    mt.Notification->Prognoses->Calibration.value.PredictedTime =
            std::chrono::system_clock::now() + std::chrono::hours(24 * 356);
}
...
void BasicGMS::initCorrection(GMS::CorrectionType_t &corr, std::string Identifier, std::string CharacteristicIdentfier,
                              double value) {
...
void
BasicGMS::InstantiateMonitoring() { mt.Monitoring->MachineTool->OperationMode = UA_MachineOperationMode::UA_MACHINEOPERATIONMODE_AUTOMATIC; }
...
void BasicGMS::InstantiateProduction() {
    ...
    mt.Production->ActiveProgram->State->CurrentState->Id =
            UA_NODEID_NUMERIC(nsFromUri(m_pServer, constants::NsMachineToolUri),
                              UA_MACHINETOOLID_PRODUCTIONSTATEMACHINETYPE_RUNNING);
}
...
void BasicGMS::Simulate() {
    ...
    if ((m_simStep % 10) < 8) {
        ...
        mt.Production->ActiveProgram->State->CurrentState->Id =
                UA_NODEID_NUMERIC(nsFromUri(m_pServer, constants::NsMachineToolUri),
                                  UA_MACHINETOOLID_PRODUCTIONSTATEMACHINETYPE_INTERRUPTED);
    }
