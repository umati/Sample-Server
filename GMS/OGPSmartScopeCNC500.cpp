...
#include "OGPSmartScopeCNC500.hpp"
...
OGPSmartScopeCNC500::OGPSmartScopeCNC500(UA_Server* pServer) : InstantiatedMachineTool(pServer) {
  MachineName = "SmartScope CNC 500";
  CreateObject();
}

void OGPSmartScopeCNC500::CreateObject() {
  std::stringstream ss;
  ss << "http://www.ogpgmbh.de/" << MachineName << "/";
...
void OGPSmartScopeCNC500::InstantiateResultManagement() {
  ...
  *result.Value->resultMetaData.resultUri = UA_String_fromChars("http://www.ogpgmbh.de/result");
...
void OGPSmartScopeCNC500::InstantiateIdentification() {
  {
    std::stringstream ss;
    ss << "http://www.ogpgmbh.de/#" << MachineName;
  ...
  mt.Identification->Manufacturer = {"", "OGP Messtechnik GmbH, Alwin Faber"};
  mt.Identification->ProductCode = "SmartScope CNC 500";
  mt.Identification->YearOfConstruction = 2022;
  mt.Identification->SoftwareRevision = "V1.0.0";
  mt.Identification->DeviceClass = "GMS";
  mt.Identification->Location = "TECHNISHOW 10 E018/TMTS 4 G0912/VIRTUAL 1 1/N 48.694241 E 9.183833";
...
void OGPSmartScopeCNC500::InstantiateMonitoring() {
...
void OGPSmartScopeCNC500::InstantiateTools() {
...
void OGPSmartScopeCNC500::Simulate() {
  ++m_simStep;
  int i = m_simStep;
  if ((m_simStep % 2) == 1) {
    SimulateStacklight();
