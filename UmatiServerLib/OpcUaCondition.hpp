...
public:
  ...
  OpcUaCondition(UA_Server *pServer, open62541Cpp::UA_NodeId source)
      :pServer(pServer), nodesMaster(pServer), Source(source)
  {
    addConditionToAddressSpace();
  }

  void Trigger()
  {
    UA_Server_triggerConditionEvent(pServer, *ConditionNodeId.NodeId, *Source.NodeId, nullptr);
  }

  ~OpcUaCondition()
  {
    if (UA_StatusCode retValConditionDelete = UA_Server_deleteCondition(pServer, *ConditionNodeId.NodeId, *Source.NodeId); retValConditionDelete != UA_STATUSCODE_GOOD)
    {
