...
namespace UmatiServerLib
{
  ...
  StateMachine::Transition_t StateMachine::readTransitionValues(open62541Cpp::UA_NodeId transition)
  {
    ...
    auto toStates = browseForChilds(m_pServer, transition, open62541Cpp::UA_NodeId(UA_NODEID_NUMERIC(0, UA_NS0ID_TOSTATE)), open62541Cpp::UA_NodeId(UA_NODEID_NUMERIC(0, UA_NS0ID_STATETYPE)));
    if (toStates.size() != 1)
    {
